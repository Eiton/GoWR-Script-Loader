...
)
{
	...
	case DLL_PROCESS_ATTACH:
	{
		...
		auto my_logger = spdlog::basic_logger_mt("logger", "GOWR-Script-Loader.log");
		spdlog::set_default_logger(my_logger);
		spdlog::flush_every(std::chrono::seconds(5));
		spdlog::info("Load winmm dll");
		my_logger->flush();
		wchar_t path[MAX_PATH];
		if (!GetWindowsDirectoryW(path, MAX_PATH)) {
			DWORD dwError = GetLastError();
			spdlog::error(std::format("Failed to get system directory:{}", dwError));
			my_logger->flush();
			return FALSE;
		}
		...
		if (syswinmm == NULL) {
			DWORD dwError = GetLastError();
			spdlog::error(std::format("Failed to load the original winmm.dll:{}", dwError));
			my_logger->flush();
			return FALSE;
		}
		setupFunctions(syswinmm);
		my_logger->flush();
		CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)loaderInit, NULL, NULL, NULL);
	}
